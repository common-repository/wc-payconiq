...
use payconiq\model\Gateway_Payconiq;
...
class Payconiq_Config {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_payconiq_check_payconiq_order_status', array( $this, 'check_payconiq_order_status' ) );
		add_action( 'wp_ajax_nopriv_payconiq_check_payconiq_order_status', array( $this, 'check_payconiq_order_status' ) );
	...
	public function add_payconiq_gateway_class( $methods ) {
		$methods[] = 'payconiq\model\Gateway_Payconiq';
		return $methods;
	}


	/**
	 * Ajax: Returns order status from payconiq to backend
	 *
	 * @since 1.0.0
	 */
	public function check_payconiq_order_status() {
		$response = array(
	...
	public function check_order_status() {
		...
		$gateway = new Gateway_Payconiq(false);
		$gateway->check_response($order_id);
